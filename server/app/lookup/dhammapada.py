...
# silently desync the runtime filter from the labels.
...
class LookupUnavailableError(Exception):
    ...
    than relaxing the filter or substituting generated content.
    """
...
def _excluded(row: dict) -> bool:
    """Crisis hard-exclusion predicate, mirrored by vocabulary.json."""
    return (
...
def _looks_like_quotation(short_reason: str, passage_text: str) -> bool:
    """True if shortReason appears to reproduce the passage."""
