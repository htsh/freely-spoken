...
Development-only check (task 2.7). Pure stdlib — no pip deps. The single
source of truth for allowed values is vocabulary.json; this script loads it
rather than hardcoding enums, so a vocabulary version bump cannot silently
...
# vocabulary.json currently lives with the OpenSpec change; it moves to
# server/app/lookup/ when the catalog is promoted (task 5.2). Override with --vocab.
DEFAULT_VOCAB = os.path.join(
    REPO, "openspec", "changes", "dhammapada-catalog-lookup", "vocabulary.json"
)
